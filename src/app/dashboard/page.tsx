...
export default function Dashboard() {
  ...
  const statusTone = reconciledCount === totalLedgers && totalLedgers > 0
    ? 'border-emerald-400/30 bg-emerald-500/10 text-emerald-100'
    : reconciledCount > 0
    ? 'border-amber-400/30 bg-amber-500/10 text-amber-100'
    : 'border-white/20 bg-white/10 text-white/70';

  const statusLabel = reconciledCount === totalLedgers && totalLedgers > 0
    ? '✅ All reconciled'
    : reconciledCount > 0
        ...
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-white/60">
              Track reconciliation progress and outstanding review work at a glance.
            </p>
          </div>
          <span className={cn('rounded-full border px-3 py-1 text-sm font-semibold', statusTone)}>{statusLabel}</span>
        </header>

        <div className="grid gap-4 md:grid-cols-3">
          <DashboardCard title="Transactions" value={summary.total.toLocaleString()} hint="Imported records" />
          <DashboardCard
            title="Needs Review"
